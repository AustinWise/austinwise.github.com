...
which means it determines the liveness of objects by coordinating with the .NET execution engine to
find live objects in the stacks of threads and in static variables.
...
> Note that these code samples are missing some error handling and object resurrection is not handled.
> You probably don't want to copy-paste. Also, these code samples use internal implementation details
> of NativeAOT that you probably should not rely upon in a regular application.
...
> for a more complete example of managing the lifetime of a `ManagedObjectWrapper`.
...
The `MyAddRef` and `MyRelease` functions allow unmanaged code to
