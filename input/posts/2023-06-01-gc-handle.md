---
layout: post.liquid
title: Lesser know CLR GC Handles
published_date: 2023-05-01 10:00:00 -0800
---
...
thread stacks, all static variables, and anywhere else the Execution Engine
stores GC pointer.

For case that don't fit nicely into the stack and static variable scanning system,
the GC provides the an interface to create GCHandles.
...
when an object has been collected.
...
can share the implementation. Besides the maintainability benefit for the .NET,
users of .NET can also benefit from smaller deploy sizes when using
[trimming](https://learn.microsoft.com/dotnet/core/deploying/trimming/trim-self-contained).
The trimming process currently has no way to reduce the size of `coreclr.dll`.
But once features have been ported to C#, the trimmer can potentially remove them.
...
is strong or not. Specifcally it calls the execution engine provided method
